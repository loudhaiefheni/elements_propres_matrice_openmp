...
//// Initialisation des constantes, des vecteurs et des matrices
	...
	long double epsilon_max = 1; //precision des resultats
	...
	gsl_vector * valeurs_propes = gsl_vector_alloc(m);
...
//1. Calculer C2k−1 = (yk, yk−1), C2k = (yk, yk) où yk = Ayk−1, for k = 1, m.
	...
	// WHILE(epsilon_max > precision_souhait
	normalize_vector(x);
...
//2. Calculer Em = B−1 m−1, Fm = Em * Bm
	...
	gsl_eigen_symmv(f_m, valeurs_propes, vecteurs_propres, my_workspace);
	gsl_eigen_symmv_free(my_workspace);
